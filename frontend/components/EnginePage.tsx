      ...
      <div className="grid gap-8 lg:grid-cols-[240px_1fr]">
        ...
        <div className="space-y-10">
          ...
          >
            ...
            <div className="absolute -top-20 right-10 h-40 w-40 rounded-full bg-neon-cyan/15 blur-[90px]" aria-hidden />
            <div className="relative space-y-6">
              <div className="glass-border inline-flex px-4 py-2 text-[0.65rem] font-semibold uppercase tracking-[0.3em] text-white/70">
                {config.title} Engine
              </div>
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="space-y-3">
                  <h1 className="text-3xl font-semibold text-white sm:text-4xl">{config.description}</h1>
                  <p className="max-w-2xl text-sm text-white/70">{config.cardDescription}</p>
              ...
              <motion.div className="flex flex-wrap gap-3" variants={staggerContainer} initial="hidden" animate="show">
                {config.features.map((feature) => (
                  <motion.span
                    key={feature}
                    variants={staggerItem}
                    whileHover={shouldReduceMotion ? undefined : { y: -3, scale: 1.02 }}
                    className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-white/80"
                  >
                    {feature}
                  </motion.span>
                ))}
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between text-xs uppercase tracking-[0.2em] text-white/60">
                        {stat.label}
                        <StatIcon className="h-4 w-4 text-neon-cyan" />
                      </div>
          ...
          >
            <div className="glass-panel p-8">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.3em] text-white/60">Gen Z flow</p>
                ...
                <button
                  type="button"
                  className="inline-flex items-center gap-2 rounded-full border border-white/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-white/70 transition hover:border-white/30"
                >
                  Try a sample
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span className="flex h-10 w-10 items-center justify-center rounded-2xl bg-white/10">
                          <ItemIcon className="h-5 w-5 text-neon-cyan" />
                        </span>
                        <div>
            ...
            <div className="glass-panel p-8">
              <p className="text-xs font-semibold uppercase tracking-[0.3em] text-white/60">Step wizard</p>
                    ...
                    >
                      ...
                      <div className="flex-1 space-y-1">
                        <div className="flex items-center gap-2 text-sm font-semibold text-white">
                          <StepIcon className="h-4 w-4 text-neon-pink" />
                          {step.title}
                        </div>
          ...
          >
            <div className="mb-6 space-y-2">
              <p className="text-xs font-semibold uppercase tracking-[0.3em] text-white/60">{config.formTitle}</p>
              <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
            <div className="hidden glass-panel p-6 lg:block">
              <p className="text-xs font-semibold uppercase tracking-[0.3em] text-white/60">Guided support</p>
            ...
            <div className="space-y-4 lg:hidden">
              <details className="glass-panel group p-5">
                <summary className="flex cursor-pointer list-none items-center justify-between text-sm font-semibold text-white">
                  Guided support
                  <ArrowUpRight className="h-4 w-4 text-white/60 transition group-open:rotate-45" />
                </summary>
