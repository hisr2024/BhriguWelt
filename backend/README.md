...
- Copy `.env.example` to `.env` to mirror local settings used in deployment
  blueprints (including `PYTHONPATH=src` and optional `SENTRY_DSN`).
...
- Endpoint request/response formats are documented in `../docs/api_reference.md`
  and the OpenAPI specification at `../docs/openapi.yaml`.
- Run `python backend/scripts/backup_data.py` before releases to snapshot the
  manuscript corpus into `backend/backups/`.
