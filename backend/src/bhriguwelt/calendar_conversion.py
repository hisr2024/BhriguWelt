...
from .astronomical_calculations import geocode_location, has_swisseph
...
class HinduCalendarContext:
    ...
    def as_payload(self) -> Dict[str, object]:
        saka_date = {
            ...
            "Month Index": self.saka_date.month_index,
            "Day": self.saka_date.day,
            "Leap-adjusted Chaitra": "Yes" if self.saka_date.leap_year else "No",
        }
        panchanga_details = {
            "Tithi": self.tithi_name,
            "Tithi Number": self.tithi_number,
            "Nakshatra": self.nakshatra,
            "Nakshatra Index": self.nakshatra_index,
            "Yoga": self.yoga,
            "Yoga Index": self.yoga_index,
            "Karana": self.karana,
            "Karana Index": self.karana_index,
            "Weekday": self.weekday,
        }
        reference_notes = {
            "Gregorian Input": {
                "Date": self.birth_date.isoformat(),
                "Time": self.birth_time.isoformat(timespec="minutes"),
                "Place": self.birth_place,
            },
            "Birth Timezone": self.birth_timezone,
            "Birth UTC Offset Minutes": self.birth_utc_offset_minutes,
            "Timezone Source": self.timezone_source,
            "IST Timestamp": self.ist_datetime.isoformat(timespec="minutes"),
            "IST Reference Longitude": self.ist_reference_longitude,
            "Conversion Factor Years": self.conversion_factor_years,
            "Ephemeris Source": self.ephemeris_source,
            "Sources": list(self.sources),
        }
        return {
            "Śaka Date": saka_date,
            "Panchanga Details": panchanga_details,
...
def convert_birth_details(birth_date: str, birth_time: str, birth_place: str) -> HinduCalendarContext:
    ...
    ist_dt, tz_notes = _ist_reference_datetime(parsed_date, parsed_time, birth_place)
    saka_date = _gregorian_to_saka(ist_dt.date())
    conversion_factor = ist_dt.date().year - saka_date.year
    panchang = _derive_panchang(ist_dt.date(), ist_dt.time())
    ...
    return HinduCalendarContext(
        ...
        birth_timezone=tz_notes["timezone_name"],
        birth_utc_offset_minutes=tz_notes["utc_offset_minutes"],
        timezone_source=tz_notes["timezone_source"],
        ist_datetime=ist_dt,
    )
...
def _ist_reference_datetime(
    gregorian_date: date, gregorian_time: time, birth_place: str
) -> Tuple[datetime, Dict[str, object]]:
    latitude, longitude, tz_name = geocode_location(birth_place)
    tz_label = tz_name or "Asia/Kolkata"
    local_tz = ZoneInfo(tz_label)
    local_dt = datetime.combine(gregorian_date, gregorian_time).replace(tzinfo=local_tz)
    ...
    return ist_dt, {
        ...
        "timezone_source": "TimezoneFinder" if tz_name else "IST fallback",
        "latitude": latitude,
        "longitude": longitude,
    }
