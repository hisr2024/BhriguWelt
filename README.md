...
- Endpoint contracts live in `docs/api_reference.md` and the machine-readable
  OpenAPI file at `docs/openapi.yaml`.
- Deployment playbooks and hosting options are in `docs/deployment.md`.
- Backup and recovery guidance is documented in
  `docs/backup_and_recovery.md` (run `python backend/scripts/backup_data.py` to
  generate timestamped copies before releases).
